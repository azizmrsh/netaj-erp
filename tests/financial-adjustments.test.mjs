...
before(async () => {
  ...
  const sale = await prisma.sale.create({ data: { invoiceNumber: `ADJ-S-${suffix}`, invoiceDate: new Date("2026-06-10"), partyId: customer.id, status: "COMPLETED", subtotal: 1000, vatAmount: 150, totalAmount: 1150 } });
  const purchase = await prisma.purchase.create({ data: { purchaseNumber: `ADJ-P-${suffix}`, purchaseDate: new Date("2026-06-11"), partyId: supplier.id, status: "COMPLETED", subtotal: 500, vatAmount: 75, totalAmount: 575 } });
...
test("الإشعار الدائن للمبيعات يخفض AR والإيراد والضريبة ويظهر في مطابقة VAT", async () => {
  const note = await prisma.$transaction((tx) => createCreditDebitNote(tx, { direction: "SALES", noteType: "CREDIT_NOTE", saleId, noteDate: "2026-06-15", amountBeforeVat: 100, vatAmount: 15, reason: "مرتجع مالي" }, "maker"));
  ...
  const snapshot = await prisma.$transaction((tx) => buildVatSnapshot(tx, new Date("2026-06-01"), new Date("2026-06-30T23:59:59.999")));
...
test("الإشعار المدين للمشتريات يزيد AP وضريبة المدخلات", async () => {
  const note = await prisma.$transaction((tx) => createCreditDebitNote(tx, { direction: "PURCHASE", noteType: "DEBIT_NOTE", purchaseId, noteDate: "2026-06-16", amountBeforeVat: 50, vatAmount: 7.5, reason: "تكلفة إضافية" }));
...
test("قيد الاستحقاق يحفظ مسودة متوازنة ثم يرحل ويُعكس مع audit", async () => {
  const adjustment = await prisma.$transaction((tx) => createAccountingAdjustment(tx, { adjustmentType: "ACCRUAL", adjustmentDate: "2026-07-10", description: "استحقاق تجريبي",
