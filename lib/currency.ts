...
export async function companyCurrency(tx: Tx) {
  ...
  const company = await tx.company.findUnique({ where: { id: companyId }, select: { id: true, tenantId: true, baseCurrencyCode: true } });
...
export async function exchangeRateAt(tx: Tx, transactionCurrency: string, date: Date) {
  ...
  const direct = await tx.exchangeRate.findFirst({
    where: { companyId: company.id, baseCurrencyCode: currency, quoteCurrencyCode: company.baseCurrencyCode, rateDate: { lte: date } },
  ...
  const inverse = await tx.exchangeRate.findFirst({
    where: { companyId: company.id, baseCurrencyCode: company.baseCurrencyCode, quoteCurrencyCode: currency, rateDate: { lte: date } },
    orderBy: [{ rateDate: "desc" }, { id: "desc" }],
  });
...
export async function saveExchangeRate(tx: Tx, input: Record<string, unknown>, userId?: number | string | null) {
  ...
  const rateDate = new Date(String(input.rateDate ?? ""));
  const rate = new Prisma.Decimal(String(input.rate ?? 0));
  if (!baseCurrencyCode || !quoteCurrencyCode || baseCurrencyCode === quoteCurrencyCode || Number.isNaN(rateDate.getTime()) || !rate.gt(0)) {
  ...
  const row = await tx.exchangeRate.upsert({
    ...
    create: { companyId: company.id, baseCurrencyCode, quoteCurrencyCode, rateDate, rate, source: String(input.source ?? "MANUAL").slice(0, 100), createdBy: userId ? String(userId) : null },
