...
export const dashboardDataSources = ["kpis.sales", "kpis.purchases", "kpis.netProfit", "kpis.liquidity", "kpis.inventory", "kpis.ar", "kpis.ap", "kpis.cashFlow", "kpis.activeCustomers", "kpis.vat", "kpis.projectProfit", "monthly", "materials", "customerActivity", "alerts", "factory", "transport"] as const;
...
export async function initializeCompanyDesign(tx: Prisma.TransactionClient, tenantId: number, companyId: number) {
  await tx.companyThemeProfile.upsert({ where: { companyId }, create: { tenantId, companyId, primaryColor: companyId === 1 ? "#b78a3d" : "#1d4ed8", secondaryColor: companyId === 1 ? "#172235" : "#0f172a", accentColor: companyId === 1 ? "#d6ab56" : "#059669", backgroundColor: "#f8f5ee", sidebarColor: "#fffdf8", sidebarStyle: "SOFT", cardStyle: "ELEVATED", chartStyle: "DIMENSIONAL" }, update: {} });
...
export async function saveTheme(tx: Prisma.TransactionClient, input: Record<string, unknown>, userId: string) {
  ...
  const data = { themePreset: presets.includes(text(input.themePreset).toUpperCase()) ? text(input.themePreset).toUpperCase() : "CORPORATE", mode: modes.includes(text(input.mode).toUpperCase()) ? text(input.mode).toUpperCase() : "LIGHT", logoUrl: safeUrl(input.logoUrl), loginLogoUrl: safeUrl(input.loginLogoUrl), primaryColor: color(input.primaryColor, "#b78a3d"), secondaryColor: color(input.secondaryColor, "#172235"), accentColor: color(input.accentColor, "#d6ab56"), backgroundColor: color(input.backgroundColor, "#f8f5ee"), sidebarColor: color(input.sidebarColor, "#fffdf8"), fontArabic: text(input.fontArabic) || "Arial", fontEnglish: text(input.fontEnglish) || "Arial", sidebarStyle: styles.includes(text(input.sidebarStyle).toUpperCase()) ? text(input.sidebarStyle).toUpperCase() : "SOFT", cardStyle: cards.includes(text(input.cardStyle).toUpperCase()) ? text(input.cardStyle).toUpperCase() : "ELEVATED", tableStyle: tables.includes(text(input.tableStyle).toUpperCase()) ? text(input.tableStyle).toUpperCase() : "STRIPED", chartStyle: charts.includes(text(input.chartStyle).toUpperCase()) ? text(input.chartStyle).toUpperCase() : "DIMENSIONAL", menuOrderJson: JSON.stringify(Array.isArray(input.menuOrder) ? input.menuOrder.map(text).filter(Boolean) : []), dashboardStyleJson: JSON.stringify(object(input.dashboardStyle)), loginBrandingJson: JSON.stringify(object(input.loginBranding)), updatedBy: userId };
