import{createCipheriv,createHash,randomBytes}from"node:crypto";import type{Prisma}from"@prisma/client";import{audit}from"@/lib/audit";import{nextDocumentNumber}from"@/lib/document-numbering";import{enqueueBackgroundJob}from"@/lib/background-jobs";
...
export async function approvalsWorkspace(tx:Prisma.TransactionClient,userId?:number,roleCodes:string[]=[]){return{requests:await tx.unifiedApprovalRequest.findMany({where:userId?{OR:[{assignedUserId:null,assignedRoleCode:null},{assignedUserId:userId},{assignedRoleCode:{in:roleCodes}}]}:undefined,include:{actions:{orderBy:{createdAt:"desc"}}},orderBy:[{status:"asc"},{createdAt:"desc"}]})}}
async function sourceDecision(tx:Prisma.TransactionClient,row:{entityType:string;entityId:number},action:string,userId:number){const status=action==="APPROVE"?"APPROVED":action==="REJECT"?"REJECTED":"DRAFT";if(row.entityType==="SALE")await tx.sale.updateMany({where:{id:row.entityId},data:{status}});else if(row.entityType==="PURCHASE")await tx.purchase.updateMany({where:{id:row.entityId},data:{status}});else if(row.entityType==="EXPENSE")await tx.expense.updateMany({where:{id:row.entityId},data:{status}});else if(row.entityType==="PROJECT_CERTIFICATE")await tx.progressCertificate.updateMany({where:{id:row.entityId},data:{status}});else if(row.entityType==="PAYROLL")await tx.payrollRun.updateMany({where:{id:row.entityId},data:{status}});else if(row.entityType==="DELIVERY_RECEIPT_NOTE")await tx.deliveryReceiptNote.updateMany({where:{id:row.entityId,status:"PENDING"},data:{status,approvedAt:action==="APPROVE"?new Date():null,approvedBy:action==="APPROVE"?String(userId):null}})}
export async function saveApproval(tx:Prisma.TransactionClient,input:Record<string,unknown>,userId:number){const action=text(input.action).toUpperCase();if(action==="REQUEST"){const entityType=text(input.entityType).toUpperCase(),entityId=Number(input.entityId),moduleKey=text(input.moduleKey).toUpperCase();return tx.unifiedApprovalRequest.upsert({where:{tenantId_companyId_moduleKey_entityType_entityId:{tenantId:Number(input.tenantId),companyId:Number(input.companyId),moduleKey,entityType,entityId}},create:{requestNumber:await nextDocumentNumber(tx,"APR",new Date()),moduleKey,entityType,entityId,entityNumber:text(input.entityNumber)||null,title:text(input.title),requestedBy:userId,amount:Number(input.amount)||null,dueAt:input.dueAt?new Date(String(input.dueAt)):null,assignedUserId:Number(input.assignedUserId)||null,assignedRoleCode:text(input.assignedRoleCode)||null},update:{title:text(input.title),status:"PENDING",assignedUserId:Number(input.assignedUserId)||null,assignedRoleCode:text(input.assignedRoleCode)||null}})}if(["APPROVE","REJECT","RETURN"].includes(action)){const row=await tx.unifiedApprovalRequest.findFirst({where:{id:Number(input.id),...(input.tenantId!==undefined?{tenantId:Number(input.tenantId)}:{}),...(input.companyId!==undefined?{companyId:Number(input.companyId)}:{})}});if(!row)throw new BusinessModuleError("طلب الموافقة غير موجود",404);if(row.status!=="PENDING")throw new BusinessModuleError("تم اتخاذ قرار على الطلب مسبقًا",409);if(row.requestedBy===userId)throw new BusinessModuleError("فصل المهام يمنع منشئ الطلب من اتخاذ القرار عليه",403);await sourceDecision(tx,row,action,userId);const status=action==="APPROVE"?"APPROVED":action==="REJECT"?"REJECTED":"RETURNED";await tx.unifiedApprovalAction.create({data:{requestId:row.id,action,comment:text(input.comment)||null,actorUserId:userId}});const updated=await tx.unifiedApprovalRequest.update({where:{id:row.id},data:{status}});await audit(tx,{action:`APPROVAL_${action}`,entityType:row.entityType,entityId:row.entityId,userId:String(userId),metadata:{requestId:row.id,comment:text(input.comment)}});return updated}throw new BusinessModuleError("إجراء الموافقات غير مدعوم")}
...
export async function savePortal(tx:Prisma.TransactionClient,input:Record<string,unknown>){const action=text(input.action).toUpperCase();if(action==="IDENTITY"){const partyId=Number(input.partyId),party=await tx.party.findUnique({where:{id:partyId}});if(!party)throw new BusinessModuleError("الطرف غير موجود");const email=text(input.email).toLowerCase();if(!/^\S+@\S+\.\S+$/.test(email))throw new BusinessModuleError("البريد الإلكتروني غير صالح");const permissions=Array.isArray(input.permissions)?input.permissions.map(String).filter(value=>["INVOICES","ORDERS","DOCUMENTS","REQUESTS"].includes(value)):[],activationToken=randomBytes(32).toString("base64url"),inviteTokenHash=createHash("sha256").update(activationToken).digest("hex"),inviteExpiresAt=new Date(Date.now()+48*60*60_000),row=await tx.portalIdentity.upsert({where:{tenantId_companyId_email:{tenantId:Number(input.tenantId),companyId:Number(input.companyId),email}},create:{partyId,email,permissionsJson:JSON.stringify(permissions),inviteTokenHash,inviteExpiresAt,status:"INVITED"},update:{partyId,permissionsJson:JSON.stringify(permissions),inviteTokenHash,inviteExpiresAt,status:"INVITED",passwordHash:null}});await tx.portalSession.deleteMany({where:{portalIdentityId:row.id}});return{...row,activationToken}}if(action==="REQUEST"){const identity=await tx.portalIdentity.findUnique({where:{id:Number(input.portalIdentityId)}});if(!identity)throw new BusinessModuleError("هوية البوابة غير موجودة");return tx.portalRequest.create({data:{portalIdentityId:identity.id,requestNumber:await nextDocumentNumber(tx,"PRQ",new Date()),requestType:text(input.requestType).toUpperCase(),subject:text(input.subject),details:text(input.details)||null}})}throw new BusinessModuleError("إجراء البوابة غير مدعوم")}
