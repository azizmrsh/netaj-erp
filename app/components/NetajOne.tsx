...
const starters = ["لخّص أداء هذا الشهر", "من المتأخر أكثر من 60 يومًا؟", "لماذا تغيّر صافي الربح؟"];
...
export default function NetajOne() {
  ...
  return <>
    ...
    {open && <div className="netaj-one-layer" role="dialog" aria-modal="true" aria-label="NETAJ ONE"><button aria-label="إغلاق NETAJ ONE" className="netaj-one-backdrop" onClick={() => setOpen(false)}/><section className="netaj-one-panel">
      ...
      {mode === "ASK" && !answer && <div className="netaj-one-starters" aria-label="أسئلة مقترحة">{starters.map(prompt => <button key={prompt} type="button" onClick={() => setValue(prompt)}>{prompt}</button>)}</div>}
