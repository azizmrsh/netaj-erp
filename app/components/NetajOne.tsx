...
type Answer = { conversationId: number; answer: string; responseType: string; providerStatus?: "READY" | "DISABLED" | "ERROR"; data?: Record<string, unknown>[]; drillDown?: string; source?: { report: string; from: string; to: string; filters?: string }; export?: { pdf?: string; excel?: string } };
type ChatMessage = { role: "USER" | "ASSISTANT"; content: string; answer?: Answer };
...
const labels: Record<string, string> = { title: "الإجراء", customer: "العميل / المورد", material: "المادة", city: "المدينة", telephone: "الهاتف", quantity: "الكمية", unitPrice: "سعر الوحدة", subtotal: "قبل الضريبة", vat: "الضريبة", total: "الإجمالي", amount: "المبلغ", bank: "البنك / الصندوق", ownership: "ملكية المخزون", nextSteps: "المسار التشغيلي", effect: "الأثر", metric: "المؤشر", value: "القيمة", sales: "المبيعات", purchases: "المشتريات", netProfit: "صافي الربح", liquidity: "السيولة", receivables: "الذمم المدينة", payables: "الذمم الدائنة", inventory: "المخزون", date: "التاريخ", invoiceNumber: "رقم الفاتورة", journalNumber: "رقم القيد", balance: "الرصيد", status: "الحالة" };
const labelsEn: Record<string, string> = { title: "Action", customer: "Customer / supplier", material: "Material", city: "City", telephone: "Phone", quantity: "Quantity", unitPrice: "Unit price", subtotal: "Subtotal", vat: "VAT", total: "Total", amount: "Amount", bank: "Bank / cash", ownership: "Stock ownership", nextSteps: "Workflow", effect: "Effect", metric: "Metric", value: "Value", sales: "Sales", purchases: "Purchases", netProfit: "Net profit", liquidity: "Liquidity", receivables: "Receivables", payables: "Payables", inventory: "Inventory", date: "Date", invoiceNumber: "Invoice number", journalNumber: "Journal number", balance: "Balance", status: "Status" };
function displayLabel(key: string, english: boolean) { return (english ? labelsEn[key] : labels[key]) ?? key.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, (letter) => letter.toUpperCase()); }
function displayValue(key: string, item: unknown) { if (typeof item === "number") return item.toLocaleString("en-US", { maximumFractionDigits: 2 }); if (item === null || item === undefined || item === "") return "—"; return String(item); }
...
export default function NetajOne() {
  ...
  async function send(event?: FormEvent) {
    ...
    setError(""); setAnswer(null); setProposal(null); setMessages((current) => [...current, { role: "USER", content: submitted }]); setState("UNDERSTANDING");
    try {
      ...
      else { setAnswer(body); setMessages((current) => [...current, { role: "ASSISTANT", content: body.answer, answer: body }]); setConversationId(body.conversationId); setValue(""); setState("COMPLETED"); }
  ...
  function speak(message = answer) { if (!message || !("speechSynthesis" in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(message.answer); utterance.lang = window.localStorage.getItem("netaj-language") === "en" ? "en-US" : "ar-SA"; window.speechSynthesis.speak(utterance); }
  function newConversation() { setConversationId(undefined); setMessages([]); setAnswer(null); setProposal(null); setValue(""); setError(""); setState("IDLE"); }
  ...
  return <>
    ...
    {open && <div className="netaj-one-layer" role="dialog" aria-modal="true" aria-label="NETAJ ONE"><button aria-label="إغلاق NETAJ ONE" className="netaj-one-backdrop" onClick={() => setOpen(false)}/><section className="netaj-one-panel">
      <header><div className="netaj-one-orb"><Sparkles size={22}/></div><div><b>NETAJ ONE</b><p>{copy.subtitle}</p></div><button type="button" onClick={newConversation} title={isEnglish ? "New conversation" : "محادثة جديدة"}><Pencil size={16}/></button><button aria-label={copy.close} onClick={() => setOpen(false)}><X/></button></header>
      ...
      {mode === "ASK" && !messages.length && <div className="netaj-one-starters" aria-label={copy.questions}>{starters.map(prompt => <button key={prompt} type="button" onClick={() => setValue(prompt)}>{prompt}</button>)}</div>}
      {mode === "ASK" && answer?.providerStatus === "DISABLED" && <div className="netaj-one-provider-status" role="status">خدمة الذكاء غير متصلة حالياً. هذه نتيجة تقريرية آمنة من نظام ERP، وليست إجابة نموذج لغوي.</div>}
      ...
      {mode === "ASK" && messages.length > 0 && <div className="netaj-one-chat" aria-live="polite">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`netaj-one-message is-${message.role.toLowerCase()}`}><span>{message.role === "USER" ? "You" : "NETAJ ONE"}</span><p>{message.content}</p>{message.answer?.source && <details><summary>{isEnglish ? "Source and scope" : "المصدر والنطاق"}</summary><small>{message.answer.source.report} · {message.answer.source.from} → {message.answer.source.to}{message.answer.source.filters ? ` · ${message.answer.source.filters}` : ""}</small></details>}{message.answer?.export && <div className="netaj-one-exports">{message.answer.export.pdf && <a href={message.answer.export.pdf}>PDF</a>}{message.answer.export.excel && <a href={message.answer.export.excel}>Excel</a>}</div>}</div>)}</div>}
      {answer && (messages.length === 0 || mode === "ACTION") && <div className="netaj-one-answer"><div><Bot size={19}/><b>{answer.answer}</b></div><button aria-label="قراءة صوتية" onClick={() => speak(answer)}><Volume2 size={18}/></button>{answer.data?.length ? <div className="netaj-one-mini-grid">{answer.data.slice(0, 4).map((row, index) => <div key={index}>{Object.entries(row).filter(([key]) => key !== "href").slice(0, 2).map(([key, item]) => <p key={key}><span>{displayLabel(key, isEnglish)}</span><b>{displayValue(key, item)}</b></p>)}</div>)}</div> : null}</div>}
