...
const groups:NavGroup[] = [
  {label:"مساحة العمل",items:[
    ...
    {label:"ملاحظات الاستلام والتسليم",href:"/notes",module:"NOTES",icon:ClipboardCheck},
...
export default function AppShell({children}:{children:React.ReactNode}){
  ...
  return <div className="erp-shell" dir={direction}>
    ...
    <aside className={`erp-sidebar ${collapsed?"is-collapsed":""} ${mobileOpen?"is-mobile-open":""}`}>
      ...
      <nav aria-label="القائمة الرئيسية" className="erp-nav">{visibleGroups.map(group=><div key={group.label} className="erp-nav-group">{!collapsed&&<p>{group.label}</p>}{group.items.map(item=>{const Icon=item.icon,active=item.href==="/"?pathname==="/":pathname.startsWith(item.href);return <Link onClick={()=>setMobileOpen(false)} title={collapsed?item.label:undefined} aria-current={active?"page":undefined} key={item.href} href={item.href} className={`erp-nav-item ${active?"is-active":""}`}><span className="erp-nav-icon"><Icon size={18}/></span>{!collapsed&&<span>{item.label}</span>}</Link>})}</div>)}</nav>
      <div className="erp-sidebar-footer"><div className="erp-trust"><ShieldCheck size={17}/>{!collapsed&&<span>بيئة مؤسسية آمنة</span>}</div></div>
    </aside>
