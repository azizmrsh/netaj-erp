...
export default function MonthlySalesReport() {
  ...
  return <section className="monthly-sales-report"><header><div><p className="text-xs font-black tracking-[.2em] text-amber-700">MONTHLY SALES REPORT</p><h2>كشف المبيعات الشهرية</h2></div><div className="flex gap-2"><label>من <input type="date" min={minDate} value={from} onChange={event => setFrom(event.target.value)} /></label><label>إلى <input type="date" min={minDate} value={to} onChange={event => setTo(event.target.value)} /></label></div></header><div className="monthly-sales-chart"><div className="monthly-sales-y-axis"><span>{compact(max)}</span><span>{compact(max / 2)}</span><span>0</span></div><div className="monthly-sales-gridlines"><i /><i /><i /><i /></div><div className="monthly-sales-bars">{rows.map((row, index) => { const value = values[index], height = Math.max(18, Math.round(value / max * 250)); return <div className="monthly-sales-bar" key={`${String(row.month)}-${index}`} title={`${monthLabel(row.month)}: ${money(value)}`}><span style={{ height }} /><small>{monthLabel(row.month)}</small><em>{compact(value)}</em></div>; })}</div><div className="monthly-sales-axis"><strong>المحور X — الأشهر</strong><strong>المحور Y — قيمة المبيعات</strong></div></div></section>;
}
