...
export default function MonthlySalesReport() {
  ...
  return <section className="monthly-sales-report"><header><div><p className="text-xs font-black tracking-[.2em] text-amber-700">MONTHLY SALES REPORT</p><h2>كشف المبيعات الشهرية</h2></div><div className="flex gap-2"><label>من <input type="date" min={minDate} value={from} onChange={event => setFrom(event.target.value)} /></label><label>إلى <input type="date" min={minDate} value={to} onChange={event => setTo(event.target.value)} /></label></div></header><div className="monthly-sales-chart"><div className="monthly-sales-gridlines"><i /><i /><i /><i /></div><div className="monthly-sales-bars">{rows.map((row, index) => { const value = values[index], cumulative = values.slice(0, index + 1).reduce((sum, item) => sum + item, 0), height = Math.max(18, Math.round(value / max * 270)); return <div className="monthly-sales-bar" key={`${String(row.month)}-${index}`} title={`${monthLabel(row.month)}: ${money(value)}`}><b>{compact(value)}</b><span style={{ height }} /><small>{monthLabel(row.month)}</small><em>{compact(cumulative)}</em></div>; })}</div></div></section>;
}
