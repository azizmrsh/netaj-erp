...
import { BarChart3, CircleDollarSign, Clock3, Gauge, Landmark, ReceiptText, ShoppingCart, WalletCards } from "lucide-react";
...
export default function SupplementaryMetrics() {
  ...
  const cards = [{ label: "إجمالي المبيعات", value: money(sales), icon: <ShoppingCart />, tone: "blue" }, { label: "إجمالي المشتريات", value: money(kpis.purchases), icon: <ShoppingCart />, tone: "purple" }, { label: "إجمالي المصاريف", value: money(kpis.expenses), icon: <ReceiptText />, tone: "amber" }, { label: "صافي الربح", value: money(netProfit), icon: <CircleDollarSign />, tone: "gold" }, { label: "السيولة البنكية", value: money(liquidity), icon: <Landmark />, tone: "blue" }, { label: "هامش الربح", value: `${margin.toFixed(1)}%`, icon: <CircleDollarSign />, tone: "gold" }, { label: "صافي التدفق النقدي", value: money(cashFlow), icon: <WalletCards />, tone: "blue" }, { label: "المتأخرات", value: `${Number((data?.alerts as Row | null)?.overdueReceivables ?? 0)} فاتورة`, icon: <Clock3 />, tone: "rose" }, { label: "رأس المال العامل", value: money(workingCapital), icon: <Gauge />, tone: "green" }, { label: "دوران المخزون", value: `${turnover.toFixed(2)} مرة`, icon: <BarChart3 />, tone: "purple" }];
  return <section className="indicator-summary-cards">{cards.map(card => <article key={card.label} className="reference-kpi"><span className={`metric-icon ${card.tone}`}>{card.icon}</span><div><p>{card.label}</p><strong>{card.value}</strong><small>{card.value.includes("%") || card.value.includes("فاتورة") || card.value.includes("مرة") ? "" : "SAR"}</small></div></article>)}</section>;
}
