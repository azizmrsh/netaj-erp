...
export default function SupplementaryMetrics() {
  ...
  const cards = [{ label: "هامش الربح", value: `${margin.toFixed(1)}%`, icon: <CircleDollarSign />, tone: "gold" }, { label: "صافي التدفق النقدي", value: sar(cashFlow), icon: <WalletCards />, tone: "blue" }, { label: "المتأخرات", value: `${Number((data?.alerts as Row | null)?.overdueReceivables ?? 0)} فاتورة`, icon: <Clock3 />, tone: "rose" }, { label: "رأس المال العامل", value: sar(workingCapital), icon: <Gauge />, tone: "green" }, { label: "دوران المخزون", value: `${turnover.toFixed(2)} مرة`, icon: <BarChart3 />, tone: "purple" }];
