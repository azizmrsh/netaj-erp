...
type InspectResult = { filename: string; headers: string[]; sheets: { name: string; headers: string[]; rowCount: number }[]; mapping: Record<string, string>; fields: Field[]; sampleRows: { sourceSheet: string; sourceRow: number; raw: Record<string, unknown> }[]; headerFingerprint: string; detectedProfile?: { id: number; name: string } | null };
...
export default function ImportsClient() {
  const [catalog, setCatalog] = useState<Target[]>([]), [batches, setBatches] = useState<Batch[]>([]), [templates, setTemplates] = useState<MappingTemplate[]>([]), [sourceSystems, setSourceSystems] = useState<SourceSystem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    if (!response.ok) throw new Error(body.error); setCatalog(body.catalog); setBatches(body.batches); setTemplates(body.templates); setSourceSystems(body.sourceSystems ?? []);
  }, []);
  useEffect(() => {
    ...
    fetch("/api/imports", { cache: "no-store", signal: controller.signal })
      ...
      .then((body) => { setCatalog(body.catalog); setBatches(body.batches); setTemplates(body.templates); setSourceSystems(body.sourceSystems ?? []); })
  ...
  return <div className="mx-auto grid max-w-7xl gap-6 p-5 lg:grid-cols-[1.4fr_0.8fr]">
    <div className="space-y-6">
      ...
      {selected && (
        <BatchDetails batch={selected} target={catalog.find((row) => row.key === selected.targetType)} busy={busy} dryRun={() => batchAction("DRY_RUN")} approve={() => batchAction("APPROVE")} execute={() => batchAction("QUEUE_EXECUTE")} rollback={() => batchAction("ROLLBACK")} />
      )}
    </div>
...
function BatchDetails({ batch, target, busy, dryRun, approve, execute, rollback }: { batch: Batch; target?: Target; busy: boolean; dryRun: () => void; approve: () => void; execute: () => void; rollback: () => void }) {
  ...
  return <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-bold">3. المعاينة والفحص والاعتماد {batch.batchNumber}</h2><p className="text-sm text-slate-500">{batch.sourceFile} · {target?.labelAr ?? batch.targetType}</p></div><Status value={batch.status}/></div>
    ...
    <div className="mb-4 flex flex-wrap gap-2">{batch.status === "PREVIEW" && <button disabled={busy || batch.invalidRows > 0} onClick={dryRun} className="rounded-lg bg-violet-700 px-5 py-2.5 font-bold text-white disabled:opacity-40">تشغيل Dry Run</button>}{batch.status === "DRY_RUN" && <button disabled={busy} onClick={approve} className="rounded-lg bg-blue-700 px-5 py-2.5 font-bold text-white disabled:opacity-40">اعتماد الاستيراد</button>}{batch.status === "APPROVED" && <button disabled={busy} onClick={execute} className="rounded-lg bg-emerald-700 px-5 py-2.5 font-bold text-white disabled:opacity-40">تنفيذ الدفعة المعتمدة</button>}{batch.status === "COMPLETED" && <button disabled={busy} onClick={rollback} className="rounded-lg border border-red-300 px-5 py-2.5 font-bold text-red-700 disabled:opacity-40">تراجع آمن</button>}</div>
