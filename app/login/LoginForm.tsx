...
export default function LoginForm() {
  ...
  return (
    <form onSubmit={submit} className="w-full max-w-md space-y-5 rounded-[28px] border border-[#e6dcc9] bg-white/95 p-8 text-[#172235] shadow-[0_30px_80px_rgba(59,45,24,.16)] backdrop-blur md:p-10">
      <div>{branding.logoUrl ? <Image src={branding.logoUrl} width={160} height={70} unoptimized alt={branding.name} className="mb-4 object-contain" />:<div className="mb-5 flex items-center gap-3"><span className="erp-brand-gem">N</span><div><b className="block tracking-[.16em]">NETAJ</b><small className="text-[#7c756a]">GLOBAL ERP</small></div></div>}<p className="text-sm font-bold" style={{ color: branding.primaryColor }}>{branding.subtitle}</p><h1 className="mt-2 text-3xl font-black">{branding.title}</h1><p className="mt-2 text-sm text-[#7c756a]">{branding.name}</p></div>
