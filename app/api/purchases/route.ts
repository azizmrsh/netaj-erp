...
export async function POST(request: Request) {
  try {
    ...
    const approvalRules = await prisma.approvalRule.findMany({ where: { entityType: "PURCHASE", isActive: true }, orderBy: { priority: "asc" } });
    ...
    return NextResponse.json({ ...purchase, requiredApprovals }, { status: 201 });
  } catch (error) {
    console.error(error);
