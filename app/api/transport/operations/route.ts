...
export async function POST(request:Request){
  try{const context=await authorizeRequest(request,{moduleKey:"TRANSPORT",action:"CREATE"}),body=await request.json() as Record<string,unknown>,type=text(body.type).toUpperCase();const result=await runWithDataScope({tenantId:context.tenantId,companyId:context.companyId},()=>prisma.$transaction(async tx=>{
    ...
    if(type==="TIRE"){
      ...
      const row=await tx.vehicleTireRecord.create({data:{truckId,vehiclePart,axle,position,serialNumber:text(body.serialNumber)||null,brand:text(body.brand)||null,size:text(body.size)||null,installedAt:date(body.installedAt),installationOdometer:body.installationOdometer?decimal(body.installationOdometer):truck!.currentOdometer,cost:decimal(body.cost),supplier:text(body.supplier)||null,notes:text(body.notes)||null}});await audit(tx,{action:"FLEET_TIRE_INSTALL",entityType:"TRUCK",entityId:truckId,userId:String(context.userId),metadata:{recordId:row.id,position}});return row;
    }
    if(type==="BATTERY"){
      ...
      const row=await tx.vehicleBatteryRecord.create({data:{truckId,position,serialNumber:text(body.serialNumber)||null,brand:text(body.brand)||null,specification:text(body.specification)||null,installedAt:date(body.installedAt),installationOdometer:body.installationOdometer?decimal(body.installationOdometer):truck!.currentOdometer,warrantyExpiry:body.warrantyExpiry?date(body.warrantyExpiry):null,supplier:text(body.supplier)||null,cost:decimal(body.cost),notes:text(body.notes)||null}});await audit(tx,{action:"FLEET_BATTERY_INSTALL",entityType:"TRUCK",entityId:truckId,userId:String(context.userId),metadata:{recordId:row.id,position}});return row;
    }
    if(type==="FUEL"){
