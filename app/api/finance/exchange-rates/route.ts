...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(await runWithDataScope(auth, () => prisma.exchangeRate.findMany({ where: { tenantId: auth.tenantId, companyId: auth.companyId }, orderBy: [{ rateDate: "desc" }, { id: "desc" }], take: 500 })));
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await runWithDataScope(auth, () => prisma.$transaction((tx) => saveExchangeRate(tx, body, auth.userId))), { status: 201 });
  } catch (error) { return response(error); }
}
