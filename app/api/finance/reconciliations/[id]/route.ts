...
import { completeBankReconciliation, deleteDraftBankReconciliation, reconciliationErrorResponse } from "@/lib/financial-reconciliation";
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const auth = await authorizeRequest(request, { moduleKey: "ACCOUNTING", action: "CANCEL" });
    const { id } = await context.params;
    if (!Number.isInteger(Number(id))) return NextResponse.json({ error: "رقم التسوية غير صحيح" }, { status: 400 });
    return NextResponse.json(await prisma.$transaction((tx) => deleteDraftBankReconciliation(tx, Number(id), auth.userId)));
  } catch (error) {
    if (error instanceof AuthError) { const response = authErrorResponse(error); return NextResponse.json({ error: response.message, code: response.code }, { status: response.status }); }
    console.error(error); const response = reconciliationErrorResponse(error); return NextResponse.json({ error: response.message }, { status: response.status });
  }
