...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const auth = await authorizeRequest(request, { moduleKey: "ACCOUNTING", action: action === "POST" ? "POST" : action === "CANCEL" ? "CANCEL" : "UPDATE" });
    if (!Number.isInteger(voucherId)) return NextResponse.json({ error: "رقم السند غير صحيح" }, { status: 400 });
    const row = await runWithDataScope(auth, () => prisma.$transaction((tx) => action === "POST" ? postVoucher(tx, voucherId, auth.userId) : action === "CANCEL" ? cancelVoucher(tx, voucherId, String(body.reason ?? "").trim() || null) : Promise.reject(new Error("INVALID_ACTION"))));
