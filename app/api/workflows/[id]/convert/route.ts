...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const source = await prisma.businessDocument.findUnique({ where: { id }, select: { direction: true } });
    if (!source) throw new WorkflowError("NOT_FOUND", "المستند غير موجود");
    await authorizeRequest(request, { moduleKey: source.direction === "PURCHASE" ? "PURCHASES" : "SALES", action: "POST" });
