...
import { PremiumEmptyState as EmptyState, PremiumSectionTitle as SectionTitle } from "./components/PremiumUI";

type Row = Record<string, unknown>;
type DashboardData = { range:{from:string;to:string}; kpis:Row; factory:Row|null; transport:Row|null; monthly:Row[]; materials:Row[]; customerActivity:Row[]; latestTransactions:Row[]; alerts:Row };
...
export default function DashboardClient({ enabledModules, companyName }:{enabledModules:string[];companyName:string}) {
  ...
  const [data,setData] = useState<DashboardData|null>(null), [message,setMessage] = useState("");
  ...
  const maxMonthly=useMemo(()=>Math.max(1,...(data?.monthly??[]).flatMap(row=>[Math.abs(Number(row.sales??0)),Math.abs(Number(row.purchases??0))])),[data]);
  ...
  return <main className="premium-dashboard mx-auto max-w-[1680px] space-y-4 p-3 md:p-5 lg:p-6">
    ...
    {!data?<DashboardSkeleton/>:<>
      ...
      <section className="premium-command-grid grid gap-4 xl:grid-cols-[1.55fr_.72fr]">
        <article className="premium-panel rounded-2xl border p-4"><SectionTitle eyebrow="PERFORMANCE" title="المبيعات والمشتريات شهريًا" description="مقارنة مباشرة مع انتقال إلى تقرير الشهر." action="/reports?report=monthly-comparison"/><div className="premium-dual-chart" aria-label="رسم المبيعات والمشتريات الشهري"><div className="premium-chart-legend"><span className="is-sales">المبيعات</span><span className="is-purchases">المشتريات</span></div><div className="premium-dual-bars">{data.monthly.map(row=><Link href={`/reports?report=monthly-comparison&from=${row.month}-01&to=${monthEnd(row.month)}`} key={String(row.month)} className="premium-dual-column" title={`${row.month} — مبيعات ${money(row.sales)}، مشتريات ${money(row.purchases)}`}><div><i className="is-sales" style={{height:`${Math.max(4,Math.abs(Number(row.sales))/maxMonthly*100)}%`}}/><i className="is-purchases" style={{height:`${Math.max(4,Math.abs(Number(row.purchases))/maxMonthly*100)}%`}}/></div><b>{String(row.month).slice(5)}</b></Link>)}</div></div>
        </article>
        <article className="premium-panel rounded-2xl border p-4"><SectionTitle eyebrow="ATTENTION" title="يحتاج انتباهك" description="استثناءات فعلية تستحق قرارًا."/><div className="premium-attention-list">{[["فواتير متأخرة",data.alerts.overdueReceivables,"/accounting?tab=reports&report=ar-aging"],["موافقات معلقة",data.alerts.pendingApprovals,"/approvals"],["تنبيهات رقابية",data.alerts.openControlAlerts,"/controls"],["مهام خلفية فاشلة",data.alerts.failedJobs,"/settings/jobs"],["تحذيرات ترحيل",data.alerts.migrationWarnings,"/imports"]].map(([label,value,href],index)=><Link key={String(label)} href={String(href)} className={`premium-alert-row ${Number(value)>0&&index<2?"is-urgent":""}`}><span>{String(label)}</span><b>{number(value)}</b><ChevronLeft size={15}/></Link>)}</div></article>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
        <article className="premium-panel rounded-2xl border p-4"><SectionTitle eyebrow="LIVE ACTIVITY" title="أحدث المعاملات" description="المستند، الطرف، القيمة والحالة من المصدر." action="/search"/>{data.latestTransactions.length?<div className="premium-activity-list">{data.latestTransactions.slice(0,6).map(row=><Link href={String(row.href)} key={String(row.key)} className="premium-activity-row"><span className="premium-activity-icon"><ReceiptText size={17}/></span><div><b>{String(row.type)} · {String(row.document)}</b><small>{String(row.party)} · {new Date(String(row.date)).toLocaleDateString("ar-SA")} · {String(row.status)}</small></div><strong>{row.unit?`${number(row.amount)} ${String(row.unit)}`:money(row.amount)}</strong><ArrowUpLeft size={14}/></Link>)}</div>:<EmptyState text="لا توجد معاملات في الفترة المختارة."/>}</article>
