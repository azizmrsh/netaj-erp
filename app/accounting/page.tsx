...
export default async function AccountingPage({ searchParams }: { searchParams: Promise<{ tab?: string; voucherType?: string }> }) {
  ...
  return <main className="finance-shell min-h-screen text-slate-900">
    ...
    <AccountingClient initialTab={query.tab} initialVoucherType={query.tab === "payment" || query.voucherType === "SUPPLIER_PAYMENT" ? "SUPPLIER_PAYMENT" : "CUSTOMER_RECEIPT"} />
  </main>;
}
